...
| **I-12-B5** | Plan §10 | All 8 dead-code candidates still present, including `components/ui/MarkdownRenderer.tsx` (unsanitized `marked` → `dangerouslySetInnerHTML`) and its only importer `components/forms/ReflectionForm.tsx`. | **STILL VALID** | Low | B10-A |
