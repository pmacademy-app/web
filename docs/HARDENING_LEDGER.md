...
**Synchronized With:** `origin/main` @ `76b96e2`; Phase 2 complete in code on
`b10a-b14b/error-boundaries-logout-ci-security` @ `208b5ec` (**not merged**)
**Date:** 2026-09-18
**Status:** B0–B6 complete and deployed · Phase 1 (B8-A…B8-E, B9-A) complete and
deployed · **Phase 2 (B7-A…B7-H, B8-F/B8-G, B9-B…B9-D, B10-A…B10-D, B11-A…B11-C,
B12-A/B12-B, B13-A/B13-B, B14-A…B14-C) complete in code on the feature branch, awaiting
merge and production verification.** B13-A requires a monitored deploy — it logs every
...
| **B7** — shared auth + route/error contract | I-07 | 🟡 **Complete in code** | All eight waves landed. **125 of 128 routes on `withRoute`**: foundation (`resolveActor`, `withRoute`), cron (6/6), auth (8/10), learner (32/32), settings (11/11), admin (61/61). The three unmigrated routes are the documented architectural exceptions — `auth/send-email-hook` and `email/webhooks` authenticate by HMAC over the raw body, `auth/callback` is redirect-only with no envelope to carry an error. B7-H's `require-with-route` lint rule is active and allows exactly those three. F-SEC-12, N-2 and N-3 closed |
| **B8** — typed data layer + DB correctness | I-08 | 🟢 **P0 correctness DEPLOYED; B8-E prepared, NOT applied** | B8-A…B8-D deployed to production at `164229a` and verified 2026-09-13: leaderboard column bug fixed (F-COR-1), authoritative XP total (F-COR-2), truncating queries bounded (F-COR-4), duplicate diagnostic run read-only (F-COR-3). `awardXp()` now treats SQLSTATE 23505 as already-awarded. **B8-E applied to production 2026-09-13 18:15:34Z** — 31 `theory_read` duplicates removed, 310 XP across 13 users, scoped partial unique index over six once-only source types. 17/17 post-migration checks passed. Executed direct-to-production by explicit approval; no staging project was used (P0-1 remains outstanding). **B8-F and B8-G complete in code** (`cd2581f`, `26ce0e8`): `lib/db/` replaces the `DBChain` escape hatch for the xp, leaderboard and remaining aggregates, and the `as unknown as` ban is active inside `lib/db/`. A repo-wide cast purge stays explicitly out of scope. See [`PHASE1_IMPLEMENTATION_TODO.md`](archive/PHASE1_IMPLEMENTATION_TODO.md) |
| **B9** — admin controls + observability | I-10 | 🟢 **B9-A deployed; B9-B…B9-D complete in code** | Out-of-band critical alerting deployed at `164229a` (F-REL-4) — **inert until `ALERT_WEBHOOK_URL` is configured in production**. B9-B threads an `x-request-id` from `proxy.ts` through `withRoute` into every incident (`c2caed5`). B9-C routes the queue, auth and webhook paths through the redacting structured logger (`8787351`) — zero raw `console.*` with exception text remain on those paths. B9-D implements the retention policy behind a dry-run gate (`309987c`) |
| **B10** — frontend API/data layer | I-11 | 🟡 **Complete in code** | B10-A corrected the error-boundary structure (F-COR-5) and introduced one canonical logout helper (F-COR-6). B10-B added the typed client over the ADR-006 envelope with a four-kind discriminated failure contract (`c3fdbe4`). B10-C adopted SWR and migrated the settings and academy call sites (`e1696c2`). B10-D migrated the admin console — **zero raw `fetch` remains anywhere in `components/admin/`** (`8fb8999`). Dead-code removal (I-12-B5) was closed by the 2026-09-13 cleanup; see the re-verification note in [`archive/README.md`](archive/README.md) |
| **B11** — design system migration | I-13 | 🟡 **Complete in code** | B11-A extended the Button/Input primitives to the variants actually in use and adopted them in the 10 highest-traffic files, and removed framer-motion from `ui/progress-bar.tsx` (`97e7bfd`). B11-B added shared learner `EmptyState`/`ErrorState` and standardised the skeleton (`ba88db9`). B11-C added the axe-core accessibility gate (WCAG 2.1 AA, fails on serious/critical, with negative tests proving it detects violations) and the `no-raw-button` ESLint rule with a 112-entry allowlist that a test keeps exactly in step with the tree (`528ed71`) |
| **B12** — marketing performance | I-14 | 🟡 **Complete in code** | B12-A measured the landing page and committed the baseline (`7ed741b`): 529.1 KB gzipped JS against a 345 KB app baseline, 171.7 KB of it framer-motion, mobile Lighthouse 44. The gate opened, so B12-B server-rendered six of the seven landing sections and removed framer-motion from the critical path (`e276995`): **321.2 KB gzip (−39%), mobile Performance 44 → 78, LCP 7.7 s → 4.1 s, TBT 1,100 ms → 416 ms, CLS held at 0, accessibility 96 → 98**. Numbers and method in [`B12A_MARKETING_PERFORMANCE_BASELINE.md`](B12A_MARKETING_PERFORMANCE_BASELINE.md) |
| **B13** — mobile/API readiness | D-01, D-04 | 🟡 **Complete in code** | B13-A made httpOnly cookies the only session store (`3535da6`): `persistSession` off, the client bridge and `AuthStateListener` deleted, `POST /api/auth/session` reduced to sign-out with server-side revocation, and no tokens in a browser login body. F-02 and F-SEC-8 closed. **⚠️ Deploying it logs every existing user out once.** B13-B established one pagination contract with a server-side cap as an abuse control and settled the versioning policy (`aae2639`) — **D-04 resolved as an explicit deferral**: `/api/v2` is a data-migration artifact, not a version boundary, and no scheme is adopted until D-01 resolves. Contract in [`API_CONTRACT.md`](API_CONTRACT.md) |
| **B14** — CI/security/E2E hardening | I-01 | 🟡 **B14-A and B14-B complete in code** | B14-A made the migration deploy fail loudly. B14-B added the `security-audit` CI job: a blocking `npm audit` gate with a reviewed, expiring allowlist, gitleaks secret scanning pinned by checksum, and Dependabot for npm and github-actions. Policy in [`SECURITY.md`](SECURITY.md) §5. The two Next.js critical advisories it blocked on were cleared by the 16.3.5 upgrade rather than allowlisted (ISSUE-25, resolved). **B14-C complete in code** (`208b5ec`): the two critical-path E2E flows (signup → verify → first lesson → XP, and login → session persistence → protected route → logout), a production-host refusal guarding the write-heavy specs, and component tests unblocked by widening the vitest `include` while keeping `environment: 'node'` as the default |
...
## Phase 2 — B7-C: Cron route migration (wave 1) — ✅ Complete in code

**Branch:** `b10a-b14b/error-boundaries-logout-ci-security` (not merged, not pushed)
**Date:** 2026-09-14

All six cron routes now go through `withRoute` with the `['cron', 'admin']` actor
policy. The duplicated bearer-token `===` preamble is gone from every one of them,
and the comparison is constant-time inside `resolveActor`.

| Route | Methods | Denial logged | Notes |
|---|---|---|---|
| `cron/cleanup` | POST, GET | no | Retention no-op response unchanged |
| `cron/daily-reminder` | POST, GET | yes | `force` flag moved into a query schema |
| `cron/process-broadcasts` | GET only | no | Kept its specific incident summary |
| `cron/process-email-queue` | POST, GET | yes | Heartbeat still written on both outcomes |
| `cron/retry-failed` | POST, GET | yes | Honesty note in the response unchanged |
| `cron/weekly-recap` | POST, GET | yes | `force` flag moved into a query schema |

### Two additions to `withRoute`, each with an immediate consumer

- **`onDenied`** — the gap B7-B recorded. Four cron routes record an unauthorized
  attempt with `logSystemError`, and that signal had to survive migration. The hook
  fires **only** on an actor-policy refusal; validation failures, rate-limit
  refusals and handler faults do not invoke it, and tests assert each of those
  negatives. It cannot influence the response — the return value is ignored and a
  rejection is swallowed and logged — and it receives the denial's internal
  `reason`, which the client never sees. It is **awaited**, unlike the previous
  fire-and-forget call: on a serverless runtime the response ends the invocation,
  so an un-awaited security log can simply be dropped.
- **`summary`** — three routes had a specific incident summary before migration.
  The summary feeds the incident fingerprint, so defaulting it would silently split
  a failure's history in two.

### Two deliberate behaviour changes

1. **401 → 403 for an authenticated non-admin.** Every cron route previously
   hardcoded 401 even when `requireAdminUser` had already answered 403, reporting
   an authenticated caller as unauthenticated. The canonical contract returns the
   correct status. Both are refusals, the scheduler is unaffected, and no client
   consumes these endpoints. One existing assertion in
   `platform-security-hardening.test.ts` pinned the old status and was updated with
   the reasoning recorded inline.
2. **`retry-failed` and `process-email-queue` no longer return the raw exception
   message** on a 500. Both rethrow so the wrapper emits the ADR-006 envelope with
   an `errorId`; the heartbeat write and the `logSystemError` record are preserved
   by keeping the route's own `catch` and rethrowing from it.

Response bodies otherwise gain only `success` and `code`, as the roadmap allows.

**Superseded test.** The "no route imports the wrapper yet" guard in
`b7b-with-route.test.ts` belonged to B7-B, when the wrapper had no consumers. It is
removed; `b7c-cron-route-migration.test.ts` now pins the exact set of route files
allowed to import the wrapper, so there is one assertion to update per wave rather
than two that can drift apart.

**Tests:** `lib/__tests__/b7c-cron-route-migration.test.ts` — 53 tests covering the
scheduler's exact call shape per route, wrong-secret and shared-prefix rejection,
