...
- **Secret Sanitization**: Error `message` strings logged via `logSystemError()` pass through `sanitizeErrorMessage()` (`lib/monitoring/logger.ts`), stripping Bearer tokens, secrets, and API keys. **Note:** this only sanitizes the `message` field — the `details` JSON object passed to `logSystemError()` is stored as-is, unsanitized. Callers must not put raw secrets into `details`.
