...
5. `content-pipeline.md` addendum: **shipped & verified** — a build-time stage renders every Mermaid diagram (fences plus `mentalModel`/`framework` top-level diagrams) to static SVG at `content:compile` time (see `Architecture-Review-Report.md §6`), styled from `theme/tokens.ts` with real Mermaid v11 engine Dagre layout and fluid responsive viewBox attributes. Disables HTML labels (`htmlLabels: false`) to enforce native SVG text/tspan rendering, and includes a precise JSDOM `getBBox()` polyfill supporting child `<tspan>` elements for accurate multi-line text layouts. Validation rule `mermaid-svg` fails any block without a compiled SVG.
