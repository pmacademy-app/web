...
- **Admin API routes are sanitized rather than genericized** (closed 2026-09-08, was D-02). `adminErrorMessage(cause, fallback)` in this module runs the message through the redaction in ADR-005 and falls back when nothing usable remains. Applied across `app/api/admin/**`; raw Supabase error fields are wrapped in `sanitizeErrorMessage()`. Admins keep the failure detail — a refused bulk requeue is actionable with the Postgres error and useless as "something went wrong" — while credentials, tokens and connection strings cannot survive. Response shapes and status codes are unchanged.
