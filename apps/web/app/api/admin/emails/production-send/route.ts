...
export const POST = withRoute(
  {
    actor: { allow: ['admin'] },
    operation: 'admin.emails.production_send.post',
    domain: 'admin',
    summary: 'Unexpected failure in POST /api/admin/emails/production-send',
  },
  async ({ request, actor }) => {
    try {
      const admin = requireAdmin(actor)
      const adminUser = { id: admin.userId, email: admin.email || 'admin@prodily.me' }
