 ...
 *
 * Anonymous by policy. No body schema is declared: the body is optional by design
 * and a missing one must fall through to the route's own "Email address is
 * required" 400 rather than a wrapper validation error.
 *
 * As with signup, the abuse controls stay in the handler because their order is
 * the security property — per-email, per-IP and aggregate limits are charged
 * before the Turnstile challenge, and all three fail closed. The success response
 * is deliberately non-enumerating (I-03-B6) and must not change.
 */
export const POST = withRoute(
  {
    actor: { allow: ['anonymous'] },
    operation: 'auth.resend_verification',
    domain: 'auth',
    summary: 'Unexpected failure while resending a verification email',
  },
  async ({ request }) => {
    try {
      ...
      // envelope with a correlating errorId. The verification-specific telemetry
      // above is preserved.
      throw err
    }
  }
