...
describe('Supabase Send Email Auth Hook Unit Test Suite', () => {
  ...
  describe('Canonical Sender Verification', () => {
    it('getFromEmail returns canonical verified domain welcome@prodily.adityagangwani.me and respects RESEND_FROM_EMAIL', () => {
      delete process.env.RESEND_FROM_EMAIL
      expect(getFromEmail()).toBe('Prodily <welcome@prodily.adityagangwani.me>')
      expect(BRAND.emailFromAddress).toBe('welcome@prodily.adityagangwani.me')

      process.env.RESEND_FROM_EMAIL = 'custom@prodily.adityagangwani.me'
      expect(getFromEmail()).toBe('Prodily <custom@prodily.adityagangwani.me>')

      delete process.env.RESEND_FROM_EMAIL
    })
  })

  describe('Canonical Callback URL Generation', () => {
    it('buildAuthCallbackUrl generates canonical callback URLs with token_hash and type', () => {
      const signupUrl = buildAuthCallbackUrl(siteUrl, 'hash_abc123', 'signup')
      expect(signupUrl).toBe('https://prodily.adityagangwani.me/api/auth/callback?token_hash=hash_abc123&type=signup&next=%2Fverified')

      const recoveryUrl = buildAuthCallbackUrl(siteUrl, 'hash_pwd456', 'recovery')
      expect(recoveryUrl).toBe('https://prodily.adityagangwani.me/api/auth/callback?token_hash=hash_pwd456&type=recovery&next=%2Freset-password%3Fmode%3Dupdate')
      ...
      expect(customRedirect).toContain('token_hash=hash_789')
      expect(customRedirect).toContain('type=signup')
      expect(customRedirect).toContain('next=%2Fcustom-destination')
    })
  })

  describe('Payload Validation', () => {
    it('POST /api/auth/send-email-hook rejects invalid JSON or missing fields', async () => {
      ...
      try {
        ...
        expect(data2.error).toContain('Missing user email')
      } finally {
  ...
  describe('Secret Verification (Production Fail-Closed)', () => {
    it('POST /api/auth/send-email-hook enforces SEND_EMAIL_HOOK_SECRET when configured or in production mode', async () => {
      ...
      expect(prodData.error).toContain('Missing hook secret configuration')
    ...
    it('POST /api/auth/send-email-hook verifies HMAC-SHA256 signatures', async () => {
    ...
    it('POST /api/auth/send-email-hook verifies v1,whsec_... Base64 HMAC signatures', async () => {
  ...
  describe('Auth Actions Execution', () => {
    it('POST /api/auth/send-email-hook executes signup & recovery email templates successfully', async () => {
      delete process.env.SEND_EMAIL_HOOK_SECRET
      try {
        const signupPayload = {
