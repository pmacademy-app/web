...
runTest('generatePersonJsonLd generates valid schema.org Person structured data', () => {
  ...
  assert.strictEqual(jsonLd.url, 'https://pmacademy.com/p/arivera')
...
runTest('formatPortfolioShareUrl returns clean canonical URL', () => {
  const url = formatPortfolioShareUrl('https://pmacademy.com/', 'arivera')
  assert.strictEqual(url, 'https://pmacademy.com/p/arivera')
})
