...
export function ConfirmDestructiveAction({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-background/80 backdrop-blur-sm animate-in fade-in-0">
      <div
        className="relative w-full max-w-lg overflow-hidden rounded-2xl border border-destructive/30 bg-card p-6 shadow-2xl space-y-6"
        role="dialog"
        aria-modal="true"
        ...
        <form onSubmit={handleConfirm} className="space-y-4">
          ...
          <div className="flex items-center justify-end gap-3 pt-2">
            <button
              ...
              onClick={handleClose}
              disabled={loading}
              className="rounded-xl border border-border px-4 py-2.5 text-xs font-semibold text-foreground hover:bg-secondary transition-colors disabled:opacity-50"
            >
              Cancel
            </button>
            <button
              ...
              className={`flex items-center justify-center gap-2 rounded-xl px-5 py-2.5 text-xs font-bold transition-all shadow-sm ${
                isDestructive
