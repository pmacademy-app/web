...
export function FellowRequestCard() {
  ...
  const { data, error, isLoading, mutate } = useApiQuery<FellowStatePayload>('/api/fellow-requests')
  ...
  const handleSubmit = async () => {
    setSubmitting(true)
    setSubmitError(null)
    const res = await apiPost<{ success: boolean; request: unknown }>('/api/fellow-requests')
    setSubmitting(false)
    ...
    if (!res.data.success) {
      setSubmitError('Failed to submit Fellow request.')
      return
    }
    ...
    return (
      <div className="rounded-2xl border border-rose-500/30 bg-rose-500/5 p-6 space-y-3">
        <div className="flex items-center gap-2 text-xs text-rose-600 dark:text-rose-400">
          ...
          <span>{error?.message || 'Failed to load Fellow status.'}</span>
        </div>
        <button
          type="button"
          onClick={handleRetry}
