...
export function CertificateCard({ certificate }: CertificateCardProps) {
  const templateVersion = certificate.templateVersion ?? 1

  if (templateVersion === 1) {
    return <CertificateCardV1 certificate={certificate} />
  }

  return <CertificateCardV2 certificate={certificate} />
}

/**
 * Template Version 1 — Pre-sprint original legacy certificate layout.
 * Preserved strictly unchanged for historical records.
 */
function CertificateCardV1({ certificate }: CertificateCardProps) {
  const {
    certificateCode,
    learnerName,
    levelInfo,
...
/**
 * Template Version 2 — Redesigned Sprint 7.3 official certificate layout.
 * Features:
 ...
 * - Embedded security checksum seal & verification metadata.
 */
function CertificateCardV2({ certificate }: CertificateCardProps) {
  const {
    certificateCode,
    learnerName,
    levelInfo,
  ...
  // QR Code encodes direct verification URL
