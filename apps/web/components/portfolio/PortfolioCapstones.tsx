...
import { Award, Calendar, CheckCircle2, MessageSquare, ChevronDown, ChevronUp } from 'lucide-react'
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              Real product deliverables authored during module milestones.
            </p>
          </div>
        </div>

        <span className="text-xs font-bold text-emerald-500 px-3 py-1 rounded-lg bg-emerald-500/10 border border-emerald-500/20">
          {capstones.length} Verified Deliverables
        </span>
      </div>
            ...
            >
              ...
              <div>
                ...
                <p className="text-xs text-primary font-medium mt-0.5">
                  Deliverable: {cap.deliverableType}
                </p>
              </div>

              {/* Content Preview / Full Text */}
              <div className="text-xs md:text-sm text-foreground/80 leading-relaxed font-sans bg-background/50 p-4 rounded-lg border border-border/50">
                {isExpanded ? (
                  <div className="whitespace-pre-wrap">{cap.content}</div>
                ) : (
                  <p className="line-clamp-4">{cap.content}</p>
                )}
              </div>
              ...
              {cap.reflection && (
                <div className="rounded-lg bg-primary/5 border border-primary/20 p-4 space-y-1.5 text-xs">
                  <div className="flex items-center gap-2 font-bold text-primary">
                    <MessageSquare className="w-3.5 h-3.5" />
                    <span>Author Reflection & Learning Note</span>
                  </div>
