  ...
  const columns: Column<TestimonialItem>[] = [
    {
      ...
      cell: (item) => (
        <div>
          <p className="font-bold text-white text-xs">{item.authorName}</p>
          <p className="text-[11px] text-slate-400 font-mono">{item.authorRole}</p>
        </div>
      ),
    },
    {
      ...
      cell: (item) => (
        <span className="px-2 py-0.5 rounded bg-slate-800 text-amber-400 font-mono text-[10px] border border-slate-700">
          {item.sourceEvent}
        </span>
      ),
    },
          ...
          {editingId === item.id ? (
            <div className="space-y-2">
              <textarea
                ...
                className="w-full text-xs p-2 rounded bg-slate-950 border border-slate-700 text-white focus:outline-none focus:border-amber-400"
                rows={3}
              />
              <div className="flex gap-2">
                <button
                  onClick={() => handleAction(item.id, 'edit', editedText)}
                  className="px-2 py-1 bg-emerald-600 hover:bg-emerald-500 text-white text-[10px] font-bold rounded flex items-center gap-1"
                >
                  <Check className="w-3 h-3" /> Save Content
                </button>
                <button
                  onClick={() => setEditingId(null)}
                  className="px-2 py-1 bg-slate-800 hover:bg-slate-700 text-slate-300 text-[10px] font-bold rounded flex items-center gap-1"
                >
    ...
    },
    {
      ...
      cell: (item) => (
        <div className="space-y-1">
          ...
          />
          {item.isPublished && (
            <span className="block px-1.5 py-0.5 rounded bg-emerald-500/10 text-emerald-400 text-[9px] font-bold border border-emerald-500/20 text-center">
              Published on Site
            </span>
          )}
        </div>
          ...
          <div className="flex items-center justify-end gap-1.5">
            ...
            {item.status !== 'approved' && (
              <button
                ...
                className="px-2 py-1 rounded bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-400 text-[10px] font-bold border border-emerald-500/20 flex items-center gap-1 transition-colors"
              >
            ...
            {item.status === 'approved' && !item.isPublished && (
              <button
                ...
                className="px-2 py-1 rounded bg-blue-500/10 hover:bg-blue-500/20 text-blue-400 text-[10px] font-bold border border-blue-500/20 flex items-center gap-1 transition-colors"
              >
            ...
            {item.isPublished && (
              <button
                ...
                className="px-2 py-1 rounded bg-slate-800 hover:bg-slate-700 text-slate-400 hover:text-white text-[10px] font-bold border border-slate-700 flex items-center gap-1 transition-colors"
              >
            ...
            {item.status !== 'rejected' && (
              <button
                ...
                className="px-2 py-1 rounded bg-rose-500/10 hover:bg-rose-500/20 text-rose-400 text-[10px] font-bold border border-rose-500/20 flex items-center gap-1 transition-colors"
              >
