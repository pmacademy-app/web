...
A) Short-term engagement metrics are, in practice, always inaccurate
B) Engagement metrics, by design, already account for content diversity
C) Repeating known-preference content reliably produces the next click
...
**2. In the Discovery Frontier model, which zone should a well-designed recommender deliberately allocate to?**
A) The Discovery Frontier
B) The Irrelevant Zone
C) Only the Known Preference Zone, exclusively
...
A) Irrelevant Zone content, in practice, always outperforms known content
B) It is, technically, impossible to recommend any irrelevant content
C) There is, in this model, no distinction between the two zones at all
D) It frustrates users with content bearing no plausible connection to interest
...
A) The total revenue directly generated by a given set of recommendations
B) How varied the recommended set is within and across recommendations
C) The raw number of distinct items currently held in the catalog
D) How quickly a given model is able to produce its predictions
...
A) A server performance issue that affects recommendation load times
B) A problem that, in practice, is unique to video streaming platforms
C) The difficulty of recommending well with little interaction history
D) An issue limited to systems built on outdated underlying technology
...
A) Onboarding preference surveys or content-based initial matching
B) Refusing to serve any recommendations until sufficient data accumulates
C) Randomly selecting recommendations with no underlying structure at all
D) Waiting indefinitely for organic data with no interim strategy at all
...
A) The recommendation model, in fact, had no measurable effect on behavior
B) The team, in practice, deliberately chose to ignore all retention data
C) Skip rate and retention, this lesson claims, always move together
...
A) A minor gap with, in practice, no real consequence for the system
B) A serious blind spot, since filter bubble damage surfaces slowly
C) That the recommender system is, in fact, performing optimally
D) That diversity metrics are, as a result, rendered unnecessary
...
A) Personalization technology is, in general, inherently unreliable
B) Users, this lesson claims, always prefer fully random recommendations
C) Excessive Known Preference weighting can shrink perceived breadth
D) Personalization, in practice, has no measurable effect on experience
...
A) Both users and catalog items are new, so little history exists for either
B) Early-stage products, in practice, have too much history to process
C) Cold-start, this lesson claims, only affects large, mature platforms
D) Early-stage products are, by law, prohibited from personalizing
...
A) A solved, static parameter that never needs revisiting again
B) An irrelevant technical detail carrying no real business impact
C) A decision made once at launch and never reconsidered afterward
D) An actively researched, continuously tuned aspect of the system
...
**12. (Scenario) A recommender's precision and click-through rate are both improving steadily, but no diversity or novelty metrics are tracked. What risk does this create?**
A) No risk; strong accuracy metrics guarantee overall recommender health
B) An undetected filter bubble may be forming beneath strong accuracy
C) The recommender is, as a result, guaranteed excellent long-term retention
D) This combination is, in practice, technically impossible to achieve
...
**13. (Product Thinking) A platform notices engagement metrics improving for six months while long-tenured users describe the experience as "repetitive." What is the most likely explanation?**
A) Qualitative feedback is unreliable and should be disregarded entirely
B) There is, in this case, no plausible connection between the two signals
C) The recommender has likely over-indexed on the Known Preference Zone
D) The platform's catalog has simply become too large to recommend well
...
**14. (Interview Reasoning) A candidate asked how to evaluate a recommender system names only click-through rate as the relevant metric. What does this most likely signal?**
A) A gap in recognizing diversity and novelty metrics are also necessary
B) A strong and genuinely complete understanding of recommender evaluation
C) That the candidate should be hired for a senior role on this basis alone
D) Nothing meaningful; click-through rate is, in fact, the only metric that matters
...
**15. (Product Thinking, Highest Difficulty) A video platform's engagement metrics have improved for six months, but qualitative feedback suggests growing user fatigue with repetitive recommendations. What is the most defensible next step?**
A) Continue the current strategy unchanged, since engagement is the only signal that matters
B) Immediately maximize randomness across all recommendations to eliminate any bubble risk
C) Dismiss the qualitative feedback as unrepresentative without further investigation
D) Introduce diversity and novelty metrics, and test increased Discovery Frontier allocation
