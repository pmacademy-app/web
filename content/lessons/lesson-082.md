...
**Apple's App Tracking Transparency (ATT)**, launched with iOS 14.5 in 2021 and confirmed directly on Apple's own developer documentation, requires every app to explicitly request a user's permission before tracking their activity across other companies' apps and websites — reversing the previous default, where cross-app tracking generally happened unless a user found and used an opt-out setting. This is data minimization enforced at the platform architecture level, not just described in a privacy policy: Apple's own developer framework makes non-consensual tracking a platform-level violation subject to App Store rejection, not merely a company's internal policy choice a product team could quietly deprioritize.

The measurable business impact of this architectural choice is directly confirmed by the company most affected by it: Meta's own CFO, Dave Wehner, told analysts on the company's Q4 2021 earnings call that ATT would cost Meta approximately $10 billion in 2022 revenue — reported consistently by CNBC and the Wall Street Journal, with the Journal noting the figure represented roughly 8% of Meta's prior-year total revenue. That number is a rare, concrete, dollar-denominated illustration of what this lesson's Collection and Sharing stage risk actually costs a business dependent on cross-app data collection when a platform-level architectural constraint removes the default flow of that data — not an abstract privacy principle, but a specific, board-reported financial consequence for the company on the losing end of the change.

*(Source: Apple's own developer documentation on the App Tracking Transparency framework, and CNBC's and the Wall Street Journal's February 2022 reporting on Meta's own stated $10 billion revenue estimate.)*
